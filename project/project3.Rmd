...
**Figure 1: Graphs a, b and c describing locations of GBRs from TSS of target genes identified.**
![](/project/project3_files/project3img2.jpg)
*Graph a shows number of associated genes per region, 80% of regions have 2 associated genes. Graph b shows the region-gene associations based on orientation and distance of the GBRs to the transcription start site (TSS) of each target gene, mostly located +/- 50 to 500kb. Graph c shows describes the same data as graph b but in absolute distance to the TSS, majority between 50 to 500 kb.*
...
```{r}
library(readr)
genomic_regions <-read_csv("/Users/ruthraichur/Desktop/website2/content/project/project3.csv")
